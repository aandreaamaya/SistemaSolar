class Jupiter extends GenericGeometry {
  constructor(gl, radius = 1, Nu = 8, Nv = 8, material = new PhongMaterial(gl), transform = Matrix4.identity(), orbitRadius = 10, orbitSpeed = 1, rotationSpeed = 1) {
  ...
  // Actualiza la posición y rotación de Júpiter en su órbita
  update(elapsed) {
    ...
    this.transform = Matrix4.multiply(Matrix4.translate(new Vector3(x, 0, z)), Matrix4.rotateY(this.phi));
  }

  // Genera los vértices de la esfera que representa a Júpiter
  getVertices() {
    let vertices = [];
    let phi;
    let theta;
  ...
  // Genera las caras de la esfera para renderizar Júpiter
  getFaces() {
    let faces = [];
